...
class RNN(Layer):
    ...
    def forward(self, inputs):
        ...
        self.outputs = np.stack(output_list, axis=0)
        return self.outputs
    ...
    def backward(self, output_gradiant):
        ...
        for i in range(self.seq_len - 1, -1, -1):
            # 计算顺序：h_grad -> z_grad -> x_grad, w_grad, b_grad
            ...
            z_gradient = self.activation.backward(h_gradient)
            total_gradient = self.linear.backward(z_gradient)
            # total_gradient 同时包含了h和x的gradient, shape == (batch, out_dim + in_dim)
            x_gradient = total_gradient[:, self.out_dim:]

            input_gradients.append(x_gradient)  # 这三个矩阵都是逆序的，其中input_gradient最后输出时需要倒一下
        ...
        # %%%%%%%%%%%%%%%%%%不计算平均值的版本%%%%%%%%%%%%%%%%%%%%%%%
        self.linear.set_gradients(w=weight_gradients, b=bias_gradients)
        list.reverse(input_gradients)
